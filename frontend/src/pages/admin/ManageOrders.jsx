...
const ManageOrders = () => {
  ...
  const handleStatusChange = async (orderId, status) => {
    if (
      status === "cancelled" &&
      !window.confirm("Cancel this order and process refund if payment is already paid?")
    ) {
      return;
    }
  ...
  const getPaymentLabel = (payment) => {
    ...
    const refundText = payment.refundStatus ? ` / Refund: ${payment.refundStatus}` : "";
    return `${payment.provider}: ${payment.status}${refundText}`;
  };
            ...
            <table className="w-full">
              <thead className="bg-slate-50">
                <tr>
                  ...
                  <th className="p-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Payment</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {orders.map((o) => (
                  <tr key={o.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="p-4">
                      <div className="text-sm text-slate-700">{getPaymentLabel(o.payment)}</div>
            ...
            {orders.map((o) => (
              <div key={o.id} className="bg-white p-5 rounded-xl shadow-sm border border-slate-200">
                ...
                <div className="mb-4 pb-4 border-b border-slate-100">
                  ...
                  <p className="text-sm text-slate-500 mt-2">{getPaymentLabel(o.payment)}</p>
                </div>
