...
const PaymentPage = () => {
  ...
  const handleCodOrder = async () => {
    ...
    setOrder(newOrder);
    setPayment({
      id: newOrder.payment?.id,
      method: "COD",
      status: newOrder.payment?.status || "COD Pending",
    });
    fetchCart();
  };
  ...
  const handleRazorpayOrder = async () => {
    ...
    const { data } = await axios.post(
      "https://prod-ecom-backend.onrender.com/api/payments/razorpay/order",
      createOrderPayload(),
      { withCredentials: true }
    );
      ...
      handler: async (response) => {
        try {
          const verifyRes = await axios.post(
            "https://prod-ecom-backend.onrender.com/api/payments/razorpay/verify",
            {
          ...
          const confirmedOrder = verifyRes.data;
          setOrder(confirmedOrder);
          setPayment({
            id: response.razorpay_payment_id,
            method: paymentMethod,
            status: confirmedOrder.payment?.status || "Paid",
          });
          fetchCart();
        } catch (err) {
