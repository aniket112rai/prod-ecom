...
const OrdersPage = () => {
  ...
  const canCancelOrder = (status) => {
    ...
    return s && !["cancelled", "shipped", "delivered"].includes(s);
  };
  ...
  const handleCancelOrder = async (orderId) => {
    ...
    try {
      await axios.put(
        `https://prod-ecom-backend.onrender.com/api/orders/${orderId}/cancel`,
        {},
        { withCredentials: true }
      );
      fetchOrders();
        ...
        <div className="space-y-4">
          {orders.map((order) => (
            ...
            >
              ...
              <div className="md:flex-shrink-0 pt-4 md:pt-0 border-t md:border-t-0 border-gray-100 flex flex-col sm:flex-row gap-2">
                <Link
                  to={`/orders/${order.id}`}
                ...
                {canCancelOrder(order.status) && (
                  <button
                    ...
                    className="block w-full md:w-auto text-center bg-red-50 text-red-600 border border-red-200 hover:bg-red-100 px-6 py-2.5 rounded-xl font-semibold transition-colors"
                  >
                    Cancel
                  </button>
                )}
